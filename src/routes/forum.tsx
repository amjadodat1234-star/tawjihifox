...
import { Heart, MessageCircle, Trash2, Send, ImageIcon, Video, Pin, X, Flame } from "lucide-react";
...
interface Comment { id: string; user_id: string; content: string; created_at: string; post_id: string }
...
function Forum() {
  ...
  const submitComment = async (postId: string, e: React.FormEvent) => {
    ...
    if (error) {
      console.error(error);
      return toast.error(error.message || "فشل إرسال التعليق");
    }
    setCommentText("");
                  ...
                  {(commentsByPost[p.id] || []).map((cm) => {
                    ...
                    return (
                      <div key={cm.id} className="flex gap-2 items-start">
                        ...
                        <div className="flex-1 bg-card rounded-2xl px-3 py-2 text-sm">
                          <div className="flex items-center justify-between gap-2 mb-0.5">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {(user?.id === cm.user_id || isAdmin) && (
                                <button onClick={() => removeComment(p.id, cm.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="h-3 w-3" /></button>
                              )}
                            </div>
                          </div>
